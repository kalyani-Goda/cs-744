...
int callback(void *myarg, int argc, char **argv, char **azColName) {
    ...
    if (argc != 1) {
	    printf("expecting 1 colum, got %d\n", argc);
	    return -1;
    }
...
int sqldb_afterRestart(sqlite3 *db) {
	char* err;
	char query[] = "SELECT requestId FROM autoGraderTable where requestStatus != 'C'";
...
int sqldb_gettoken(sqlite3 *db, int *token) {
	...
	if ((rc != SQLITE_DONE) && (rc != SQLITE_ROW)) {
    		printf("get token execution failed: %s\n", sqlite3_errmsg(db));
...
int sqldb_insertRequest(sqlite3 *db, int token, char *filename) {
	sqlite3_stmt* stmt;
	char query[]= "INSERT INTO autoGraderTable(requestId, filename) VALUES(?, ?)";
	...
	rc = sqlite3_step(stmt);
	if (check_error(rc) != SQLITE_OK) {
    		printf(" insert request execution failed: %s\n", sqlite3_errmsg(db));
    		sqlite3_finalize(stmt);
    		return -1;/* failure */;
	}
...
int sqldb_deleteRequest(sqlite3 *db, int token) {
	...
	rc = sqlite3_step(stmt);
	if (check_error(rc) != SQLITE_OK) {
    		printf("delete request execution failed: %s\n", sqlite3_errmsg(db));
    		sqlite3_finalize(stmt);
    		return -1;/* failure */;
	}
...
int sqldb_updateStatus(sqlite3 *db, int token, int type, char *newValue) {
	...
	rc = sqlite3_step(stmt);
	if (check_error(rc) != SQLITE_OK) {
    		printf("udate status execution failed: %s\n", sqlite3_errmsg(db));
    		sqlite3_finalize(stmt);
    		return -1;/* failure */;
	}
...
int sqldb_updateTime(sqlite3 *db, int token, double start, double end) {
	...
	rc = sqlite3_step(stmt);
	if (check_error(rc) != SQLITE_OK) {
    		printf("update time execution failed: %s\n", sqlite3_errmsg(db));
    		sqlite3_finalize(stmt);
    		return -1;/* failure */;
	}
...
int sqldb_getStatus(sqlite3 *db, int token, int type, char *retValue) {
	...
	rc = sqlite3_step(stmt);
	if ((rc != SQLITE_DONE) && (rc != SQLITE_ROW)) {
    		printf("get result status execution failed: %s\n", sqlite3_errmsg(db));
    		sqlite3_finalize(stmt);
    		return -1;/* failure */;
	}
