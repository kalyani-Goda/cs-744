...
int checkUid(int uid) {
		for(int i = 0; i< gparam.nusers; i++) {
				if(gparam.uids[i] == uid)
						return 0;
		}
		return -1;
}
...
int main(int argc, char* argv[]) {
    ...
    if (argc < 5) {
        fprintf(stderr, "Usage: Server port_number config_file grader_thread_pool_size status_thread_pool_size\n");
        exit(1);
    }
    ...
    while (TRUE) {
        ...
        */
		...
		int uid, request_type = -1;
        if (newsockfd < 0) {
            error("ERROR on accept");
        }
        else {
			int err = recv_request(newsockfd, &request_type, &uid);
			if(err != 0) {
			...
			err = checkUid(uid);
			if(err != 0) {
					printf("wrong userid received %d\n", uid);
					close(newsockfd);
					continue;
			}
            request_received++;
		...
		if(request_type == USER_NEW_REQUEST) {
				pthread_t pt;
				struct file_request *request = (struct file_request *)calloc(sizeof(struct file_request),1);
				if(request == NULL) {
				...
				int rc = add_to_status_queue(db, newsockfd, uid, GetTime());
				if(rc != 0) {
