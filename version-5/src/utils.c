...
int send_file(int sockfd, char * file_path) {
	...
	if(!file)
	{
                printf("Failed to send open file %s %d\n", file_path, errno);
...
int recv_file(int sockfd, char * file_path) {
	...
	if(!file)
	{
        printf("Failed to open recv file %s %d\n", file_path, errno);
		perror("Error opening file \n");
		return -1;
	}
...
int recv_request(int sockfd, int *request_type, int *uid) {
	
	if(sockfd < 0) {
		printf("Sockfd is wrong\n");
		return -1;
	}
	char file_size_bytes[MAX_FILE_SIZE_BYTES*2];
	if(recv(sockfd, file_size_bytes, sizeof(file_size_bytes), 0) == -1)
	{
		perror("Error receiving file size\n");
		return -1;
	}
	memcpy(request_type, file_size_bytes, MAX_FILE_SIZE_BYTES);
	memcpy(uid, &file_size_bytes[MAX_FILE_SIZE_BYTES], MAX_FILE_SIZE_BYTES);

	return 0;
}

int send_request(int sockfd, int request_type, int uid) {
	...
	char file_size_bytes[MAX_FILE_SIZE_BYTES*2];
	memcpy(file_size_bytes, &request_type, MAX_FILE_SIZE_BYTES);
	memcpy(&file_size_bytes[MAX_FILE_SIZE_BYTES], &uid, MAX_FILE_SIZE_BYTES);
	if(send(sockfd, file_size_bytes, sizeof(file_size_bytes), 0) == -1)
	{
		perror("Error sending msg size\n");
		return -1;
